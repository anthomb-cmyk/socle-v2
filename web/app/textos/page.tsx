...
export default async function TextosPage() {
  ...
  const [phonesRes, eventContactsRes, eventLeadsRes, dealsRes] = await Promise.all([
    ...
    sb
      .from("deals")
      .select("id,title,stage,contact_name,contact_phone")
...
function buildConversations(
  ...
  for (const event of events) {
    ...
    const lead = event.related_lead_id
      ...
      : null;
    const deal = findDeal(number, payload, ctx.deals);
...
function findDeal(number: string, payload: Record<string, unknown>, deals: DealRow[]) {
