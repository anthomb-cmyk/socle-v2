...
<Response>
  <Dial answerOnBridge="true" timeout="20" record="record-from-answer-dual" recordingStatusCallback="${escapeXml(recordingCallbackUrl)}" recordingStatusCallbackMethod="POST">
    ...
    // Fire-and-forget logging — wrapped in try/catch so it can NEVER block or throw
    if (callLogId) {
      try {
        const { createSupabaseAdminClient } = await import("@/lib/supabase-server");
        const sb = createSupabaseAdminClient();
        void sb.from("call_logs")
          .update({ raw: { bridge_attempted_at: new Date().toISOString(), lead_name: leadName } })
          .eq("id", callLogId);
      } catch { /* if supabase import fails, still return TwiML */ }
    }

    return twimlResponse(twiml);

  } catch (err) {
    // Last-resort catch — Twilio must always get a valid TwiML response
    console.error("[outbound-bridge] Unhandled error:", err);
    return twimlResponse(
      `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Say language="fr-CA" voice="alice">Erreur inattendue. Fin de l'appel.</Say>
</Response>`
    );
  }
}
