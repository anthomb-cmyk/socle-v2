/**
 * company-website.ts — Company website researcher for Pipeline A.
 *
 * Uses Brave Search to find a company's website, then fetches the HTML and
 * extracts phone numbers from the page body.  Each found phone becomes a
 * non-authoritative EvidenceCandidate.
 */
...
/**
 * Research a company's website for phone numbers.
 *
 * Query: `"${entity.legal_name}" Quebec contact OR site:.ca`
 * Fetches the top MAX_PAGES results and extracts NANP phones.
 */
export async function companyWebsiteResearcher(
  sb: AnyClient,
  owner: CanonicalOwnerRow,
  target: ReqEntity,
): Promise<EvidenceCandidate[]> {
  const query = `"${target.legal_name}" Quebec contact OR site:.ca`;

  let results: Awaited<ReturnType<typeof braveSearch>>;
  try {
    results = await braveSearch(query, MAX_PAGES * 2);
  } catch (err) {
    console.error("[company-website] braveSearch failed:", err);
    return [];
  }
